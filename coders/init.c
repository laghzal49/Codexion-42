...
/*   Updated: 2026/04/12 15:55:00 by tlaghzal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_coders(t_sim *sim)
{
	...
	while (i < sim->params.num_coders)
	{
		...
		next_idx = (i + 1) % sim->params.num_coders;
		sim->coders[i].right_dongle = &sim->dongles[next_idx];
		if (pthread_mutex_init(&sim->coders[i].time_mutex, NULL) != 0)
			return (1);
		if (pthread_cond_init(&sim->coders[i].time_cond, NULL) != 0)
			return (1);
		i++;
	}
	return (0);
